...
const loadLanguages = async () => {
	try {
		...
		for (const path in localeFiles) {
			...
			if (code) {
				// Aquí decimos que localeData es un objeto indexable con 'language' opcional
				const localeData = localeFiles[path] as Record<string, any>;

				// Chequeamos que language sea un objeto antes de acceder a 'name'
				const languageName =
					localeData.language && typeof localeData.language === "object"
						? ((localeData.language.name as string | undefined) ?? code)
						: code;
