<template>
  <BaseModal v-model="store.isAddInstanceModalOpen" :title="languageStore.getTranslation('Launcher.addInstance.title')">
    <div class="mt-6 space-y-6">
      ...
      <div class="space-y-2">
        <label class="block text-sm font-medium text-stone-300">
          {{ languageStore.getTranslation('Launcher.addInstance.memory') }}
        </label>
        <div class="flex gap-4">
          <div class="flex-1">
            <input
              v-model="formData.minMemory"
              type="number"
              class="w-full px-3 py-2 bg-stone-700 border border-stone-600 rounded-md text-stone-200 focus:outline-none focus:border-stone-500"
              :placeholder="languageStore.getTranslation('Launcher.addInstance.minMemory')"
            >
          </div>
          <div class="flex-1">
            <input
              ...
              type="number"
              class="w-full px-3 py-2 bg-stone-700 border border-stone-600 rounded-md text-stone-200 focus:outline-none focus:border-stone-500"
              :placeholder="languageStore.getTranslation('Launcher.addInstance.maxMemory')"
            >
          </div>
        </div>
      </div>
