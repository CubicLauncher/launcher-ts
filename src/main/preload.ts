...
const cubic = {
  ...
  launcher: {
    ...
    downloadVersion: async (version: string): Promise<void> => {
      try {
        await ipcRenderer.invoke('download-version', version);
      } catch (error) {
    ...
    launchVersion: async (version: string): Promise<void> => {
      try {
  ...
  events: {
    ...
    onDownloadProgress: (callback: (progress: number) => void): void => {
      ipcRenderer.on('download-progress', (_event: Electron.IpcRendererEvent, progress: number) => {
        callback(progress);
      });
    },
    ...
    onLauncherProgress: (callback: (status: string) => void): void => {
      ipcRenderer.on('launcher-progress', (_event: Electron.IpcRendererEvent, status: string) => { // Aquí usamos Electron.IpcRendererEvent
        callback(status);
      });
    },
    ...
    onDownloadComplete: (callback: () => void): void => {
      ipcRenderer.on('download-complete', (_event: Electron.IpcRendererEvent) => {
        callback();
      });
    },
  },
